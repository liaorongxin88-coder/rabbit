...
>
> **当前基线：代码、测试和 `manual-verification-checklist.md` 用于判断实际行为，飞书状态在版本上线后同步。** 2026-08-29，线上推进到 v1.0.10 后，已将 16 条遗漏的已交付记录回写为「验收中」；飞书仍不能代替源码和设备验收证据。
...
- [ ] **E. 暂不回写**（2026-08-25 的旧决定，已被后续指令取代）
- [ ] A. 全部回写为「已完成」，并附源码证据到详细说明字段
- [ ] B. 只回写 9 条 P0 BUG，其余 8 条留待人工确认
- [ ] C. 我自己改，不需要代写
- [ ] D. 先出一份回写预览给我看，确认后再执行
- [x] **F. 在版本上线后把已交付记录回写为「验收中」**（2026-08-29 执行）。当前分布为已完成 45、验收中 35、方案设计中 1；唯一未进入验收的是 F13 批次统计。

---

## 三、17 条待改状态清单

> 本节和第五节保留 2026-08-25 的审计快照，不再代表当前待办。当前状态以页首基线和 `execution-plan.md` 5.13 为准。
