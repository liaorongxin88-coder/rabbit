...
Compose 中的 MySQL 默认不映射到宿主机端口，只供 Compose 网络内的 backend 访问。
...
直接从 Maven 启动后端时，先准备一个能从宿主机访问的 MySQL，并通过 Spring 数据源环境变量指向该实例。根 Compose 的 MySQL 默认没有宿主机端口映射。
...
source ../.env
set +a
export SPRING_DATASOURCE_URL='jdbc:mysql://127.0.0.1:3306/rabbit_app?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=Asia/Shanghai&allowPublicKeyRetrieval=true'
...
默认配置在 `backend/rabbit-boot/src/main/resources/application.yml`，生产或多人环境用环境变量覆盖数据库、JWT 和管理员 bootstrap 配置。
...
java -jar rabbit-boot/target/rabbit-backend.jar
```
...
Flutter 代码结构和规则见 `app/.rule`、[App 文档](../app/README.md) 与 [源码 README](../../app/README.md)。
...
Admin 工程规则见 `admin/.rules`、[Admin 文档](../admin/README.md) 与 [源码 README](../../admin/README.md)。

## 联调入口

需要同时联调三端时，先用 Compose 启动 backend，再分别启动 Admin 或 Flutter。后端 API E2E、Admin 浏览器脚本和 Android 真机脚本见 [测试和验证](testing.md)。
