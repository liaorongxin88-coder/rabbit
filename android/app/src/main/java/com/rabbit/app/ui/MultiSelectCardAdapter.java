...
public class MultiSelectCardAdapter extends ArrayAdapter<String> {
    private final List<Boolean> checked = new ArrayList<Boolean>();

    public MultiSelectCardAdapter(@NonNull Context context, @NonNull List<String> objects) {
    ...
    public void toggle(int position) {
        ...
        checked.set(position, !Boolean.TRUE.equals(checked.get(position)));
        notifyDataSetChanged();
    }

    public boolean isChecked(int position) {
        return position >= 0 && position < checked.size() && Boolean.TRUE.equals(checked.get(position));
    }

    public List<Integer> getCheckedPositions() {
        List<Integer> res = new ArrayList<Integer>();
        for (int i = 0; i < checked.size(); i++) {
            if (Boolean.TRUE.equals(checked.get(i))) {
                res.add(i);
            }
        }
        return res;
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        View v = convertView;
        if (v == null) {
            v = LayoutInflater.from(getContext()).inflate(R.layout.item_card_check_two_line, parent, false);
        }
