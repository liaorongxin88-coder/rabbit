...
import com.rabbit.app.mapper.ParturitionRecordMapper;
import com.rabbit.app.mapper.PrepartumRecordMapper;
import com.rabbit.app.mapper.PregnancyCheckRecordMapper;
...
import com.rabbit.app.mapper.WeaningRecordMapper;
import com.rabbit.app.mapper.WeaningRecordAllocationMapper;
...
public class BatchService {
    ...
    public List<Batch> listBatchesPage(Long houseId, String q, int page, int pageSize) {
        if (page <= 0) {
            page = 1;
        }
        if (pageSize <= 0) {
    ...
    @Transactional
    public Batch createBatch(Long userId, Long houseId, String batchCode, List<Long> femaleRabbitIds, String remark, String requestId) {
        ...
        try {
            ...
            try {
                ...
                Batch dup = batchMapper.selectByHouseAndRequestId(houseId, requestId);
                if (dup != null) {
                    requestDedupService.markDone(houseId, userId, api, requestId);
                    return dup;
                }
            ...
            for (Long rid : femaleRabbitIds) {
                ...
                if (!batchRabbitMapper.selectActiveByRabbit(houseId, rid).isEmpty()) {
    ...
    @Transactional
    public void mating(Long userId, Long houseId, Long batchId, Long femaleRabbitId, Long maleRabbitId, Date matingDate, String requestId) {
        ...
        GlobalSetting gs = requireSetting(houseId);

        BatchRabbit br = batchRabbitMapper.selectActiveByBatchAndRabbit(houseId, batchId, femaleRabbitId);
        if (br == null) {
        ...
        Date nextDate = DateUtil.plusDays(matingDate, gs.getPalpationDays());

        int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "已配种", matingDate, nextDate, "摸胎", maleRabbitId, String.valueOf(userId));
        if (rows <= 0) {
            throw new BizException(409, "状态已变化，请刷新后重试");
        }

        if (batch.getStartDate() == null) {
            batchMapper.updateStatusAndDates(houseId, batchId, "进行中", matingDate, batch.getEndDate(), String.valueOf(userId));
        }
    ...
    @Transactional
    public void pregnancyCheck(Long userId, Long houseId, Long batchId, Long rabbitId, Date checkDate, String result, String remark, String requestId) {
        ...
        BatchRabbit br = batchRabbitMapper.selectActiveByBatchAndRabbit(houseId, batchId, rabbitId);
        if (br == null) {
        ...
        if ("怀孕".equals(result)) {
            ...
            Date nextDate = DateUtil.minusDays(dueDate, gs.getPrepartumDays());
            int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "怀孕确认", br.getLastEventDate(), nextDate, "备产", br.getMaleRabbitId(), String.valueOf(userId));
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            breedingPerformanceMapper.incBreedingResult(houseId, rabbitId, true);
            insertHistory(userId, batchId, rabbitId, fromStatus, "怀孕确认", "摸胎", record.getId(), "pregnancy_check_records");
            requestDedupService.markDone(houseId, userId, api, requestId);
            return;
        }
        if ("空怀".equals(result)) {
            int rows = batchRabbitMapper.deactivateIfActive(houseId, br.getId(), DateUtil.now(), "空怀退出批次", String.valueOf(userId));
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            breedingPerformanceMapper.incBreedingResult(houseId, rabbitId, false);
            insertHistory(userId, batchId, rabbitId, fromStatus, "空怀", "摸胎", record.getId(), "pregnancy_check_records");
            checkAndCompleteBatch(houseId, batchId, userId, DateUtil.now());
            requestDedupService.markDone(houseId, userId, api, requestId);
            return;
        }

        int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "不确定", br.getLastEventDate(), br.getNextEventDate(), br.getNextEventType(), br.getMaleRabbitId(), String.valueOf(userId));
        if (rows <= 0) {
            throw new BizException(409, "状态已变化，请刷新后重试");
        }
        insertHistory(userId, batchId, rabbitId, fromStatus, "不确定", "摸胎", record.getId(), "pregnancy_check_records");
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
            throw e;
    ...
    @Transactional
    public void prepartumFinish(Long userId, Long houseId, Long batchId, Long rabbitId, Date actionDate, String remark, String requestId) {
        ...
        Date dueDate = DateUtil.plusDays(br.getLastEventDate(), 30);
        int rows = batchRabbitMapper.updateNextEventIfStatus(houseId, br.getId(), br.getCurrentStatus(), dueDate, "分娩", String.valueOf(userId));
        if (rows <= 0) {
            throw new BizException(409, "状态已变化，请刷新后重试");
        }
        insertHistoryAt(userId, batchId, rabbitId, br.getCurrentStatus(), br.getCurrentStatus(), "备产完成", actionDate, record.getId(), "prepartum_records");
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
            throw e;
    ...
    @Transactional
    public void parturition(Long userId, Long houseId, Long batchId, Long rabbitId, Date birthDate, int totalKits, int liveKits, boolean failed, String remark, String requestId) {
        ...
        breedingPerformanceMapper.ensureExists(houseId, rabbitId);
        breedingPerformanceMapper.addParturition(houseId, rabbitId, totalKits, liveKits, birthDate);
        ...
        if (failed) {
            ...
            int rows = batchRabbitMapper.deactivateIfActive(houseId, br.getId(), now, "分娩失败退出批次", op);
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            ...
            if (oldCage != null && houseId.equals(oldCage.getHouseId())) {
                ...
                cageMapper.updateRabbitCountAndStatus(houseId, oldCage.getId(), newCount, status, op);
            }

            rabbitMapper.updateDeparture(houseId, rabbitId, now, "parturition_fail", op);
            ...
            insertHistory(userId, batchId, rabbitId, fromStatus, "分娩失败", "分娩", record.getId(), "parturition_records");
        ...
        int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "哺乳中", birthDate, nextDate, "断奶", br.getMaleRabbitId(), String.valueOf(userId));
        if (rows <= 0) {
            throw new BizException(409, "状态已变化，请刷新后重试");
        }
        insertHistory(userId, batchId, rabbitId, fromStatus, "哺乳中", "分娩", record.getId(), "parturition_records");
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
            throw e;
    ...
    @Transactional
    public void weaning(Long userId, Long houseId, Long batchId, Long rabbitId, Date weaningDate, int weaningCount, Integer maleCount, Integer femaleCount, Long targetCageId, Double avgWeight, String remark, String requestId) {
        ...
        Cage cage = null;
        List<WeaningRecordAllocation> allocations = new ArrayList<WeaningRecordAllocation>();
        if (weaningCount > 0) {
            if (targetId != null) {
        ...
        String fromStatus = br.getCurrentStatus();
        Date nextDate = DateUtil.plusDays(weaningDate, gs.getPostpartumDays());
        int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "休整期", weaningDate, nextDate, "催情", br.getMaleRabbitId(), String.valueOf(userId));
        if (rows <= 0) {
            throw new BizException(409, "状态已变化，请刷新后重试");
        }
        insertHistory(userId, batchId, rabbitId, fromStatus, "休整期", "断奶", record.getId(), "weaning_records");
        ...
        for (WeaningRecordAllocation a : allocations) {
            ...
            int oldCount = c.getRabbitCount() == null ? 0 : c.getRabbitCount();
            String newStatus = "0".equals(c.getStatus()) ? "3" : c.getStatus();
            cageMapper.updateRabbitCountAndStatus(houseId, c.getId(), oldCount + add, newStatus, String.valueOf(userId));
    ...
    private List<WeaningRecordAllocation> pickCommodityCageAllocations(Long houseId, int count) {
        ...
        List<WeaningRecordAllocation> rows = new ArrayList<WeaningRecordAllocation>();
        int left = count;
    ...
    private int allocToCage(Cage c, int left, List<WeaningRecordAllocation> rows) {
    ...
    @Transactional
    public void aphrodisiacStart(Long userId, Long houseId, Long batchId, List<Long> rabbitIds, String requestId) {
        ...
        for (Long rabbitId : rabbitIds) {
            ...
            int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "催情中", now, null, null, br.getMaleRabbitId(), String.valueOf(userId));
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            insertHistory(userId, batchId, rabbitId, fromStatus, "催情中", "催情开始", null, null);
        }
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
    ...
    @Transactional
    public void aphrodisiacFinish(Long userId, Long houseId, Long batchId, List<Long> rabbitIds, String requestId) {
        ...
        for (Long rabbitId : rabbitIds) {
            ...
            int rows = batchRabbitMapper.updateStatusAndEventIfStatus(houseId, br.getId(), fromStatus, "待配种", now, next, "配种", br.getMaleRabbitId(), String.valueOf(userId));
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            insertHistory(userId, batchId, rabbitId, fromStatus, "待配种", "催情完成", null, null);
        }
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
    ...
    @Transactional
    public void sale(Long userId, Long houseId, Long batchId, List<Long> rabbitIds, Date saleDate, String remark, String requestId) {
        ...
        for (Long rabbitId : rabbitIds) {
            ...
            if (br.getNextEventType() != null && !"出售".equals(br.getNextEventType())) {
                throw new BizException(400, "当前计划事件不允许出售");
            }

            rabbitMapper.updateDeparture(houseId, rabbitId, saleDate, "0", String.valueOf(userId));
            int rows = batchRabbitMapper.deactivateIfActive(houseId, br.getId(), saleDate, remark == null ? "出售" : remark, String.valueOf(userId));
            if (rows <= 0) {
                throw new BizException(409, "状态已变化，请刷新后重试");
            }
            ...
            if (cage != null && houseId.equals(cage.getHouseId())) {
                ...
                cageMapper.updateRabbitCountAndStatus(houseId, cage.getId(), newCount, status, String.valueOf(userId));
            }

            insertHistory(userId, batchId, rabbitId, br.getCurrentStatus(), "已出售", "出售", null, null);
        }
    ...
    @Transactional
    public void completeBatch(Long userId, Long houseId, Long batchId, Date endDate, boolean force, String remark, String requestId) {
        ...
        if (active != 0) {
            ...
            batchRabbitMapper.deactivateByBatch(houseId, batchId, x, remark == null ? "手动结束批次" : remark, String.valueOf(userId));
        }
        batchMapper.updateStatusAndDates(houseId, batchId, "已完成", b.getStartDate(), x, String.valueOf(userId));
        requestDedupService.markDone(houseId, userId, api, requestId);
        } catch (RuntimeException e) {
            requestDedupService.markFailed(houseId, userId, api, requestId, e.getMessage());
            throw e;
    ...
    public List<ReplacementRecord> listDueReplacement(Long houseId, boolean onlyUnnotified) {
        if (onlyUnnotified) {
    ...
    private GlobalSetting requireSetting(Long houseId) {
    ...
    private void insertHistory(Long userId, Long batchId, Long rabbitId, String fromStatus, String toStatus, String reason, Long relatedId, String relatedTable) {
        RabbitStatusHistory h = new RabbitStatusHistory();
    ...
    private void insertHistoryAt(Long userId, Long batchId, Long rabbitId, String fromStatus, String toStatus, String reason, Date changeTime, Long relatedId, String relatedTable) {
        RabbitStatusHistory h = new RabbitStatusHistory();
        h.setRabbitId(rabbitId);
        h.setBatchId(batchId);
        h.setFromStatus(fromStatus);
    ...
    private String pickKidGender(int index, int total, int maleCount, int femaleCount) {
