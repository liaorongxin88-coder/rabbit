...
export function WorkspaceFarmsPage() {
  ...
  return (
    <>
      <PageHeader
        title="兔场管理"
        description={workspace.selectedHouse?.name ?? '创建或选择一个兔场'}
        actions={
          <>
            {workspace.selectedHouse ? <HousePermissionBadge permission={workspace.permission} /> : null}
            <HouseDialog
              mode="create"
              open={createOpen}
              onOpenChange={setCreateOpen}
              disabled={!workspace.canCreateHouse}
      ...
      {workspace.houses.length === 0 ? (
        <Empty>
          ...
          <EmptyDescription>
            {workspace.canCreateHouse ? '创建兔场后会自动生成初始笼位。' : '请通过手机号邀请加入已有兔场。'}
          </EmptyDescription>
        </Empty>
      ) : (
        <Card>
