...
export function MerchantOutboundDialog({
  houseId,
  disabled,
  canControl,
  onSaved,
    ...
    setSelected((current) => {
      ...
      return {
        ...
        [rabbitId]: {
          ...
          selectionType: rabbit.eligibility === 'EARLY_SALE' ? 'EARLY_SALE' : 'NORMAL',
          earlySaleReason: '',
        },
      }
    })
  ...
  async function freezeSelection(event: React.FormEvent<HTMLFormElement>) {
    ...
    const missingReason = selectedItems.some(
      (item) => item.selectionType === 'EARLY_SALE' && !item.earlySaleReason?.trim(),
    )
    if (missingReason) {
    ...
    try {
      ...
      setTask(frozen)
      setSelected(Object.fromEntries(frozen.selectedItems.map((item) => [item.rabbitId, item])))
      setPhase('confirm')
    } finally {
      setBusy(false)
    }
  ...
  async function submit() {
    ...
    try {
      const stateVersions = Object.fromEntries(
        task.selectedItems.map((item) => [String(item.rabbitId), item.stateVersion]),
      )
      const earlySaleReasons = Object.fromEntries(
        task.selectedItems
          .filter((item) => item.selectionType === 'EARLY_SALE')
          .map((item) => [String(item.rabbitId), item.earlySaleReason?.trim() ?? '']),
      )
  ...
  async function adjustConflicts() {
    ...
    const remaining = task.selectedItems.filter((item) => !conflictIds.has(item.rabbitId))
    setBusy(true)
    try {
      const refreshed = await precheckOutboundTask(task.houseId, task.taskId)
      setTask(refreshed)
      setSelected(Object.fromEntries(remaining.map((item) => [item.rabbitId, item])))
      setResult(null)
      setPhase('select')
    } finally {
      setBusy(false)
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => void handleOpenChange(nextOpen)}>
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>出库资格、销售快照和并发状态均归属当前兔场。</DialogDescription>
        </DialogHeader>
        ...
        {busy && !task ? (
          ...
          <form className="flex flex-col gap-4" onSubmit={freezeSelection}>
            ...
            <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
              {task.rabbits.map((rabbit) => {
                const selectable = rabbit.eligibility === 'NORMAL' ||
                  ...
                  <div key={rabbit.rabbitId} className="rounded-md border p-3">
                    <label className="flex items-start gap-3">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block text-xs text-muted-foreground">
                          ...
                          {!selectable && rabbit.recommendedAction ? ` · ${rabbit.recommendedAction}` : ''}
                        </span>
                      </span>
                    </label>
                    {checked && rabbit.eligibility === 'EARLY_SALE' ? (
                      <Input
                        ...
                        maxLength={200}
                        required
                        onChange={(event) => updateEarlyReason(rabbit.rabbitId, event.target.value)}
                      />
                    ) : null}
                  </div>
                )
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                ...
                <Input id="outbound-date" type="date" value={saleDate} required onChange={(event) => setSaleDate(event.target.value)} />
              </Field>
              <Field>
                <FieldLabel htmlFor="outbound-weight">总重量（kg）</FieldLabel>
                <Input id="outbound-weight" type="number" min="0.01" step="0.01" value={totalWeight} required onChange={(event) => setTotalWeight(event.target.value)} />
              </Field>
              <Field>
                <FieldLabel htmlFor="outbound-price">单价（元/kg）</FieldLabel>
                <Input id="outbound-price" type="number" min="0" step="0.01" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} />
              </Field>
              <Field>
                <FieldLabel htmlFor="outbound-customer">客户</FieldLabel>
                <Input id="outbound-customer" value={customer} maxLength={100} onChange={(event) => setCustomer(event.target.value)} />
              </Field>
            </div>
            <Field>
              <FieldLabel htmlFor="outbound-remark">备注</FieldLabel>
              <Textarea id="outbound-remark" value={remark} maxLength={500} onChange={(event) => setRemark(event.target.value)} />
            </Field>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => void handleOpenChange(false)}>取消</Button>
              <Button type="submit" disabled={busy || selectedItems.length === 0 || Number(totalWeight) <= 0}>
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-md border p-4">
              <div className="text-sm font-medium">本次出库 {task.selectedItems.length} 只</div>
              <div className="mt-2 flex flex-wrap gap-2">
                {task.selectedItems.map((item) => <Badge key={item.rabbitId} variant="outline">兔 #{item.rabbitId}</Badge>)}
              </div>
            </div>
            <div className="grid gap-3 text-sm sm:grid-cols-2">
              <Review label="销售日期" value={saleDate} />
              <Review label="总重量" value={`${totalWeight} kg`} />
              <Review label="单价" value={unitPrice ? `${unitPrice} 元/kg` : '-'} />
              <Review label="客户" value={customer || '-'} />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" disabled={busy} onClick={() => setPhase('select')}>返回调整</Button>
          ...
          <div className="flex flex-col gap-4">
            {result.status === 'COMPLETED' ? (
              <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
                <div className="flex items-center gap-2 font-medium"><CheckCircle2Icon className="size-5" />出库完成</div>
                <div className="mt-2 text-sm">销售单 {result.saleOrderNumber ?? `#${result.saleOrderId}`} · {result.rabbitCount} 只 · {result.totalWeight ?? totalWeight} kg</div>
              </div>
            ) : (
              <div className="rounded-md border border-destructive/30 bg-destructive/5 p-4">
                <div className="flex items-center gap-2 font-medium text-destructive"><AlertTriangleIcon className="size-5" />出库状态冲突</div>
                <div className="mt-3 max-h-52 space-y-2 overflow-y-auto">
                  {result.conflicts.map((conflict) => (
                    <div key={`${conflict.rabbitId}-${conflict.errorCode}`} className="rounded-md border bg-background p-3 text-sm">
                      <div className="font-medium">兔 #{conflict.rabbitId} · {conflict.currentState}</div>
                      <div className="mt-1 text-muted-foreground">{conflict.message} · {conflict.recommendedAction}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <DialogFooter>
              {result.status === 'COMPLETED' ? (
                <Button type="button" onClick={() => void handleOpenChange(false)}>完成</Button>
              ) : (
                <>
                  <Button type="button" variant="outline" onClick={() => void handleOpenChange(false)}>取消</Button>
                  <Button type="button" disabled={busy} onClick={() => void adjustConflicts()}>
                    {busy ? <Spinner data-icon="inline-start" /> : null}
                    移除冲突项
                  </Button>
                </>
...
function Summary({ label, value }: { label: string; value: number }) {
  return <div className="rounded-md border px-3 py-2"><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 text-lg font-semibold">{value}</div></div>
}

function EligibilityBadge({ value }: { value: OutboundEligibility }) {
  const variant = value === 'BLOCKED' ? 'destructive' : value === 'NORMAL' ? 'default' : 'secondary'
