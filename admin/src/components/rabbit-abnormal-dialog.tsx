...
const abnormalStatuses = ['外伤', '采食异常', '精神萎靡', '疑似疾病', '其他异常']
...
export function RabbitAbnormalDialog({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : '新增异常记录失败，请稍后重试')
    } finally {
      setSaving(false)
    }
  }
        ...
        <form className="contents" onSubmit={handleSubmit}>
          <FieldGroup className="overflow-y-auto py-1">
            <Field>
              ...
              <Select value={warningStatus} onValueChange={setWarningStatus} disabled={saving}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving}>
              {saving ? <Spinner data-icon="inline-start" /> : <CameraIcon data-icon="inline-start" />}
              记录异常
            </Button>
          </DialogFooter>
        </form>
