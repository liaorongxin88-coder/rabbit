...
export function RabbitFormDialog({
  ...
  const [batchEntryResult, setBatchEntryResult] =
    useState<BatchRabbitEntryResult | null>(null);
  ...
  useEffect(() => {
    ...
    const initialCage = cages.find(
      (cage) =>
        cage.id === initialCageId && cage.isEnabled && cage.houseId === houseId,
    );
    const defaultCage =
      initialCage ??
      cages.find((cage) => cage.isEnabled && cage.houseId === houseId);
  ...
  useEffect(() => {
    if (!open) return;
    setBatchId((current) =>
      keepValidProductionBatchId(current, availableBatches),
    );
  ...
  const requiredFacts = new Set(
    selectedEntry?.requiredFacts.map((fact) => fact.fact) ?? [],
  );
  const needsMatingDate =
    requiredFacts.has("MATING_DATE") || requiredFacts.has("GESTATION_ANCHOR");
  const currentCage = cages.find((cage) => cage.id === rabbit?.cageId);
  const selectedCage = cages.find((cage) => cage.id === Number(cageId));
  const cageValidationMessage =
    !rabbit && cageId && houseId
      ? rabbitCageValidationMessage(selectedCage, type, houseId)
      : null;
  const quantityValue = Number(quantity);
  const isBatchEntry =
    !rabbit && Number.isInteger(quantityValue) && quantityValue > 1;
  ...
  function resetReproductiveStage(nextType: string, nextGender: string) {
    ...
    setReproductiveStage((current) =>
      options.some((option) => option[0] === current)
        ? current
        : (options[0]?.[0] ?? ""),
    );
  }
  ...
  async function handleSubmit(event: React.SubmitEvent<HTMLFormElement>) {
    ...
    if (
      isBatchEntry &&
      (!Number.isFinite(Number(totalWeight)) ||
    ...
    if (
      motherId &&
      (!Number.isInteger(Number(motherId)) || Number(motherId) <= 0)
    ) {
    ...
    const entry =
      canOpenReproEntry && selectedEntry
        ? buildRabbitReproEntryInput({
    ...
    if (selectedEntry) {
      const missing = selectedEntry.requiredFacts.find((fact) => {
        ...
        if (fact.fact === "MATING_DATE" || fact.fact === "GESTATION_ANCHOR")
          return !matingDate;
      ...
      if (missing) {
        toast.error(
          `从【${selectedEntry.stageLabel}】入轨需要补录${missing.label}`,
        );
        return;
      }
    }
      ...
      if (rabbit) {
        ...
        if (result.skippedCages.length > 0) {
          ...
          if (result.enteredRabbitCount > 0) {
            toast.success(
              `已录入 ${result.enteredRabbitCount} 只，未录入项见表单内结果`,
            );
          } else {
        ...
        toast.success(
          selectedEntry
            ? `兔只已录入，并从【${selectedEntry.stageLabel}】入轨`
            : "兔只已录入",
        );
        onOpenChange(false);
      }
          ...
          <FieldGroup className="overflow-y-auto pr-1">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                ...
                <Select
                  ...
                  disabled={Boolean(rabbit)}
                >
                  <SelectTrigger id="rabbit-type">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      {Object.entries(rabbitTypeLabels).map(
                        ([value, label]) => (
              ...
              <Field>
                ...
                <Select
                  ...
                  disabled={Boolean(rabbit)}
                >
                  <SelectTrigger id="rabbit-gender">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {reproductiveStageOptions.length === 0 ? (
                ...
                <Field>
                  <FieldLabel htmlFor="rabbit-reproductive-stage">
                    繁殖阶段
                  </FieldLabel>
                  <Select
                    value={reproductiveStage}
                    onValueChange={setReproductiveStage}
                  >
                    <SelectTrigger id="rabbit-reproductive-stage">
                      <SelectValue placeholder="未填写" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectGroup>
            ...
            {canOpenReproEntry ? (
              <>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field>
                    <FieldLabel htmlFor="rabbit-repro-stage">
                      生产阶段
                    </FieldLabel>
                    <Select
                      value={reproStage}
                      onValueChange={handleReproStageChange}
                    >
                      <SelectTrigger id="rabbit-repro-stage">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          <SelectItem value={NO_REPRO_ENTRY}>
                            暂不入轨
                          </SelectItem>
                          {entryPoints.map((entryPoint) => (
                            <SelectItem
                              key={entryPoint.stage}
                              value={entryPoint.stage}
                            >
                  ...
                  <Field>
                    <FieldLabel htmlFor="rabbit-stage-entered-at">
                      进入该阶段日期
                    </FieldLabel>
                    <Input
                      ...
                      onChange={(event) =>
                        setStageEnteredAt(event.target.value)
                      }
                    />
                  </Field>
                </div>
                {selectedEntry ? (
                  <>
                    <Field>
                      <FieldLabel htmlFor="rabbit-production-batch">
                        生产批次
                      </FieldLabel>
                      <Select
                        value={batchId}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      {needsMatingDate ? (
                        <Field>
                          <FieldLabel htmlFor="rabbit-mating-date">
                            配种日期
                          </FieldLabel>
                          <Input
                            ...
                            value={matingDate}
                            onChange={(event) =>
                              setMatingDate(event.target.value)
                            }
                          />
                        </Field>
                      ) : null}
                      {requiredFacts.has("BIRTH_DATE") ? (
                        <Field>
                          <FieldLabel htmlFor="rabbit-birth-date">
                            分娩日期
                          </FieldLabel>
                          <Input
                      ...
                      {requiredFacts.has("LIVE_KITS") ? (
                        <Field>
                          <FieldLabel htmlFor="rabbit-live-kits">
                            活仔数
                          </FieldLabel>
                          <Input
                            id="rabbit-live-kits"
                            ...
                            value={liveKits}
                            onChange={(event) =>
                              setLiveKits(event.target.value)
                            }
                          />
                        </Field>
                      ) : null}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                ...
                >
                  <SelectTrigger id="rabbit-source">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
            ...
            {!rabbit && arrivalMethod === "1" ? (
              <Field>
                <FieldLabel htmlFor="rabbit-mother-id">
                  母兔 ID（可选）
                </FieldLabel>
                <Input
                  id="rabbit-mother-id"
            ...
            {!rabbit ? (
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {isBatchEntry ? (
                  <Field>
                    <FieldLabel htmlFor="rabbit-total-weight">
                      总重量（kg）
                    </FieldLabel>
                    <Input
            ...
            <div className="border bg-secondary p-3 text-sm">
              ...
              {batchEntryResult.skippedCages.length > 0 ? (
                <p className="mt-1 text-muted-foreground">
                  未录入：
                  {batchEntryResult.skippedCages
                    .map(
                      (item) =>
                        `${item.cageNumber || `#${item.cageId}`}：${item.rabbitCount}只，${item.reason}`,
                    )
                    .join("；")}
                </p>
              ) : null}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                saving ||
                !cageId ||
                Boolean(cageValidationMessage) ||
                Boolean(selectedEntry && !batchId)
              }
            >
...
export function RabbitTransferDialog({
  ...
  function selectByExactNumber(value: string) {
    ...
    const matches = cages.filter(
      (cage) =>
        cage.cageNumber.toLowerCase() === keyword ||
        String(cage.id) === keyword,
    );
    if (matches.length !== 1) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      const result = await transferRabbitCage(
        houseId,
        rabbit.id,
        Number(targetCageId),
      );
      const target =
        cages.find((cage) => cage.id === result.toCageId)?.cageNumber ??
        `#${result.toCageId}`;
      toast.success(
        result.mode === "SWAP"
      ...
      <DialogContent>
        ...
        <FieldGroup>
          <Field>
            ...
            >
              <SelectTrigger id="transfer-target-cage">
                <SelectValue placeholder="选择目标笼位" />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  {options.map((cage) => (
                    <SelectItem key={cage.id} value={String(cage.id)}>
                      ...
                      {cage.rabbitCount} 只{isSwapTarget(cage) ? " · 对调" : ""}
                    </SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
          ...
          <Field>
            ...
            {numberHint ? (
              <p
                ...
                data-testid="transfer-number-hint"
              >
                {numberHint}
              </p>
            ) : null}
...
export function RabbitReplacementDialog({
  ...
  const targetCages = useMemo(
    () =>
      houseId
        ? cages.filter((cage) => isReplacementTargetCage(cage, houseId))
        : [],
    [cages, houseId],
  );
...
export function RabbitDepartureDialog({
  ...
  const [departureType, setDepartureType] =
    useState<RabbitDepartureType>("death");
  const [reason, setReason] = useState("");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      pendingRequest.current = null;
      toast.success(
        `兔 #${rabbit.id} 已${departureType === "cull" ? "淘汰" : "登记死亡"}`,
      );
      onOpenChange(false);
      await onSaved();
    } catch {
      ...
      <DialogContent>
        ...
        <FieldGroup>
          <Field>
            ...
            >
              <SelectTrigger id="livestock-departure-type">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
          ...
          <Field>
            <FieldLabel htmlFor="livestock-departure-reason">
              离场原因
            </FieldLabel>
            <Input
          ...
          <Field>
            <label
              className="flex items-start gap-3 text-sm"
              htmlFor="livestock-departure-confirm"
            >
              <input
