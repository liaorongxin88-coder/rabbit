  ...
  try {
    ...
    page.on('console', (message) => {
      const isExpectedNetworkFailure =
        expectStatisticsNetworkFailure && message.text().includes('net::ERR_FAILED')
    ...
    await page.route((url) => {
      ...
      return requestUrl.origin === baseUrl && requestUrl.pathname.startsWith('/api/')
    }, async (route) => {
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'desktop.png'), fullPage: true })

    await page.setViewportSize(NARROW)
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'narrow.png'), fullPage: true })
    const documentWidth = await page.evaluate(() => document.documentElement.scrollWidth)
    assert.ok(documentWidth <= NARROW.width, 'narrow viewport has horizontal overflow')

    scenario = 'empty'
    await page.reload()
    await page.getByText('当前没有进行中的批次。建立批次后会显示统计。').waitFor()
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'empty.png'), fullPage: true })

    scenario = 'network-failure'
    expectStatisticsNetworkFailure = true
    await page.reload()
    await page.getByRole('alert').getByText('批次统计读取失败，请重试。').waitFor()
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'network-failure.png'), fullPage: true })
