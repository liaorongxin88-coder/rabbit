...
test("forces failed parturition counts to zero", () => {
  assert.deepEqual(normalizeParturitionCounts(true, 8, 6), {
    totalKits: 0,
    liveKits: 0,
  });
  assert.deepEqual(normalizeParturitionCounts(false, 8, 6), {
    totalKits: 8,
    liveKits: 6,
  });
  assert.deepEqual(normalizeParturitionPayload(true, 8, 6), {
    failed: true,
    totalKits: 0,
    liveKits: 0,
  });
  assert.deepEqual(normalizeParturitionPayload(false, 8, 6), {
    failed: false,
    totalKits: 8,
    liveKits: 6,
  });
...
test("reuses rabbit departure request only while its draft is unchanged", () => {
  ...
  const first = getOrCreateRabbitDepartureRequest(
    null,
    payload,
    createRequestId,
  );
  const retry = getOrCreateRabbitDepartureRequest(
    first,
    { ...payload },
    createRequestId,
  );
  const changed = getOrCreateRabbitDepartureRequest(
    retry,
    { ...payload, eventType: "death" },
    createRequestId,
  );
...
test("normalizes generic batch action payloads before comparing retries", () => {
  ...
  assert.equal(
    batchActionPath(21, "aphrodisiac/start"),
    "/api/batches/21/aphrodisiac/start",
  );
});

test("builds deferred weaning separation paths", () => {
  assert.equal(
    pendingWeaningRecordsPath(21),
    "/api/batches/21/weaning-records",
  );
  assert.equal(
...
test("reuses generic batch action requestId only for the same batch action draft", () => {
  ...
  const first = getOrCreateBatchActionRequest(
    null,
    {
      batchId: 21,
      action: "weaning",
  ...
  const retry = getOrCreateBatchActionRequest(
    first,
    {
      batchId: 21,
      action: "weaning",
  ...
  const changedPayload = getOrCreateBatchActionRequest(
    retry,
    {
      batchId: 21,
      action: "weaning",
  ...
  const changedAction = getOrCreateBatchActionRequest(
    changedPayload,
    {
      batchId: 21,
      action: "parturition",
  ...
  const changedBatch = getOrCreateBatchActionRequest(
    changedAction,
    {
      batchId: 22,
      action: "parturition",
  ...
  const afterSuccessClear = getOrCreateBatchActionRequest(
    null,
    {
      batchId: 22,
      action: "parturition",
  ...
  const separation = getOrCreateBatchActionRequest(
    null,
    {
      batchId: 34,
      action: "separation",
  ...
  const separationRetry = getOrCreateBatchActionRequest(
    separation,
    {
      batchId: 34,
      action: "separation",
  ...
  const separationChanged = getOrCreateBatchActionRequest(
    separationRetry,
    {
      batchId: 34,
      action: "separation",
