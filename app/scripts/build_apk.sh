...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/build_apk.sh <dev|test|prod> [--debug|--profile|--release] [extra flutter build apk args...]

Examples:
  ./scripts/build_apk.sh dev --debug
  ./scripts/build_apk.sh test --debug
  ./scripts/build_apk.sh prod --release --split-per-abi
USAGE
  rabbit_env_help
}
...
"$RABBIT_FLUTTER_BIN" build apk "$build_mode" \
