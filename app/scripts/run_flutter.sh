...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/run_flutter.sh <dev|test|prod|release> [--debug|--profile|--release] [extra flutter run args...]

Examples:
  ./scripts/run_flutter.sh dev -d emulator-5554
  ./scripts/run_flutter.sh test --debug
  ./scripts/run_flutter.sh prod --profile -d R58N...
USAGE
  rabbit_env_help
}
...
"$RABBIT_FLUTTER_BIN" run "$run_mode" \
